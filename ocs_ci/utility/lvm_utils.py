...
def osd_scenario1(osd, devices_dict, dmcrypt=False):
    ...
    vgname = vgcreate(
        osd, osd.LvmConfig.vg_name % "1", devices_dict.get("device1")
    )  # dev is a single device
    data_lv1 = lvcreate(
        osd,
        osd.LvmConfig.data_lv % "1",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(20),
    )
    data_lv2 = lvcreate(
        osd,
        osd.LvmConfig.data_lv % "2",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(20),
    )
    data_lv3 = lvcreate(
        osd,
        osd.LvmConfig.data_lv % "3",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(20),
    )
    data_lv4 = lvcreate(
        osd,
        osd.LvmConfig.data_lv % "4",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(20),
    )
    db_lv1 = lvcreate(
        osd,
        osd.LvmConfig.db_lv % "1",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(8),
    )
    db_lv2 = lvcreate(
        osd,
        osd.LvmConfig.db_lv % "2",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(8),
    )
    wal_lv1 = lvcreate(
        osd,
        osd.LvmConfig.wal_lv % "1",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(2),
    )
    wal_lv2 = lvcreate(
        osd,
        osd.LvmConfig.wal_lv % "2",
        osd.LvmConfig.vg_name % "1",
        osd.LvmConfig.size.format(2),
    )
...
def osd_scenario2(osd, devices_dict, dmcrypt=False):
    ...
    scenario = (
        "{{'data':'{vdb1}','db':'{vdb2}','wal':'{vdb3}'}},{{'data':'{vdc}',"
        "'db':'{vdb4}','wal':'{vdb5}'}},{{'data':'{vdd}'}}"
    ).format(
        vdb1=devices_dict.get("device1") + "1",
        vdb2=devices_dict.get("device1") + "2",
        vdb3=devices_dict.get("device1") + "3",
        vdc=devices_dict.get("device2"),
        vdb4=devices_dict.get("device1") + "4",
        vdb5=devices_dict.get("device1") + "5",
        vdd=devices_dict.get("device3"),
    )
...
def osd_scenario3(osd, devices_dict, dmcrypt=False):
    ...
    scenario = (
        "{{'data':'{vdb}','db':'{dblv1}','db_vg':'{vgname}','wal':'{wallv1}',"
        "'wal_vg':'{vgname}'}},"
        "{{'data':'{datalv1}','data_vg':'{vgname}',"
        "'db':'{vdd2}','wal':'{vdd3}'}},{{'data':'{vdd1}'}}"
    ).format(
        vdb=devices_dict.get("device1"),
        dblv1=db_lv1,
        vgname=vgname,
        wallv1=wal_lv1,
        datalv1=data_lv1,
        vdd1=devices_dict.get("device3") + "1",
        vdd2=devices_dict.get("device3") + "2",
