...
def post_deploy_vault_verification():
    """
...
def validate_external_vault():
    ...
    # TODO 3. check storagecluster CR for 'kms' enabled

    Raises:
        NotFoundError : if key not found in vault
    ...
    if config.ENV_DATA.get("use_vault_namespace"):
        ...
        os.environ["VAULT_NAMESPACE"] = vault.vault_namespace
        vault.get_vault_backend_path()

    kvlist = vault_kv_list(vault.vault_backend_path)
