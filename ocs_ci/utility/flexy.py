...
def load_cluster_info(cluster_ctx=None):
    """
    Parse cluster_info.json and cluster_info.yaml files generated by Flexy and
    load relevant configuration.

    Args:
        cluster_ctx (MultiClusterConfig): config object of a specific cluster in case of multiclsuter
    ...
    cluster_info_file = os.path.join(
        cluster_ctx.ENV_DATA["cluster_path"], "cluster_info.json"
    )
    if os.path.exists(cluster_info_file):
        ...
        if (
            ...
            cluster_ctx.DEPLOYMENT["disconnected"] = True
            cluster_ctx.DEPLOYMENT["mirror_registry"] = cluster_info.get(
                "MIRROR_REGISTRY"
            )
            cluster_ctx.DEPLOYMENT["mirror_registry_user"] = cluster_info.get(
                "MIRROR_REGISTRY_LOGIN_USER"
            )
            cluster_ctx.DEPLOYMENT["mirror_registry_password"] = cluster_info.get(
                "MIRROR_REGISTRY_LOGIN_PASSWORD"
            )
            cluster_ctx.DEPLOYMENT["int_svc_instance"] = cluster_info.get(
                "INT_SVC_INSTANCE_PUBLIC_IP"
            )

    cluster_info_file = os.path.join(
        cluster_ctx.ENV_DATA["cluster_path"], "cluster_info.yaml"
    )
    if os.path.exists(cluster_info_file):
        logger.info(f"File {cluster_info_file} exists, loading relevant info.")
        with open(cluster_info_file) as f:
        ...
        if cluster_info.get("INSTALLER", {}).get("CLIENT_PROXY"):
            ...
            cluster_ctx.ENV_DATA["client_http_proxy"] = cluster_ctx.ENV_DATA.get(
                "client_http_proxy", client_proxy
            )
            os.environ["http_proxy"] = cluster_ctx.ENV_DATA["client_http_proxy"]
            os.environ["https_proxy"] = cluster_ctx.ENV_DATA["client_http_proxy"]
