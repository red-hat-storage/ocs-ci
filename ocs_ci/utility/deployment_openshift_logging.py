...
def get_lokistack_subscription():
    """
    This function will check creation of subscription for lokistack operator
    """
...
def create_obc(yaml_file, resource_name, skip_resource_exists=False):
    """Creation of OBC to fetch details for secret."""
...
def install_logging():
    ...
    assert ocp_logging_obj.create_obc(
        yaml_file=constants.LO_OBC_YAML, resource_name=constants.OBJECT_BUCKET_CLAIM
    )
    # Creating secret
    # waiting for few minutes so that configmap and secrets get created
    time.sleep(180)
    bucket_host_cmd = (
        f"oc get -n {constants.OPENSHIFT_LOGGING_NAMESPACE} configmap"
    ...
    # creates lokistack
    ocp_logging_obj.create_lokistack(yaml_file=constants.LO_LOKISTACK_YAML)
    ...
    # co_subscription_yaml["spec"]["channel"] = logging_channel
