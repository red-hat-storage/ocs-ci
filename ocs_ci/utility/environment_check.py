...
def compare_dicts(before, after):
    ...
    added = []
    removed = []
    uid_before = [uid['metadata']['name'] for uid in before]
    uid_after = [uid['metadata']['name'] for uid in after]
    ...
    if diff_added:
        added = [
            val for val in after if val['metadata']['name'] in [
                v for v in diff_added
            ]
        ]
    if diff_removed:
        removed = [
            val for val in before if val['metadata']['name'] in [
                v for v in diff_removed
            ]
        ]
