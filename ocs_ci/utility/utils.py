...
def get_git_active_branch(repo_path: str = ".") -> str:
    """
    Retrieve the currently active Git branch name for the specified repository.

    This function uses GitPython to determine the active (checked-out) branch
    in a local Git repository. If the repository is in a detached HEAD state,
    it will log a warning and return None.
    ...
    Returns:
        Optional[str]: The name of the active branch (e.g., "master", "main", "feature-x"),
            or an empty string if the repository is in a detached HEAD state.

    Raises:
        git.exc.InvalidGitRepositoryError: If the specified path is not a valid Git repository.
        git.exc.NoSuchPathError: If the provided path does not exist.
        git.exc.GitCommandError: If there is an error running a Git command.
        Exception: For any unexpected error that occurs while retrieving the branch name.
    """
    try:
        repo = git.Repo(repo_path)
        if repo.head.is_detached:
            log.warning("Git HEAD is detached. No active branch found.")
            return ""

        active_branch = repo.active_branch.name
        log.info(f"Active Git branch: {active_branch}")
        ...
        log.warning(f"Git repository error while retrieving branch name: {ex}")
        raise
