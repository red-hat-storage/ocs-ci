...
def ceph_health_recover(health_status, namespace=None):
    ...
    Raises:
        CephHealthExceptionNotRecovered: When Ceph health was not recovered
        CephHealthExceptionRecovered: When Ceph health was recovered
    ...
    for fix_dict in ceph_health_fixes:
        ...
        if re.search(pattern, health_status):
            log.info(
                "Trying to fix Ceph Health becasue we found in Health status the matching pattern"
                f": '{pattern}'!"
            )
            fix_dict["func"](
            ...
            try:
                ...
                log.error("")
                raise CephHealthExceptionNotRecovered(
                    f"Attempt to try to recover the Ceph Health failed! Exception: {ex}"
                )
            raise CephHealthExceptionRecovered(
...
def ceph_health_check(namespace=None, tries=20, delay=30, fix_ceph_health=False):
    ...
    Args:
        ...
        fix_ceph_health (bool): If True, it will try to fix the health to be OK
            even if it will recover, we will get an exception CephHealthExceptionRecovered
