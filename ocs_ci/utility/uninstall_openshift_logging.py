...
from ocs_ci.helpers.helpers import (
    fetch_used_size, default_ceph_block_pool, verify_volume_deleted_in_backend
)
...
def uninstall_cluster_logging():
    ...
    logger.info(f"Used space before deletion of cluster logging {used_space_before_deletion}")
    ...
    for pvc_name, uuid in pvc_uuid_map.items():
        rbd = verify_volume_deleted_in_backend(
            interface=constants.CEPHBLOCKPOOL, image_uuid=uuid,
            pool_name=cbp_name
        )
        assert rbd, (
    ...
    logger.info(f"Used space after deletion of cluster logging {used_space_after_deletion}")
