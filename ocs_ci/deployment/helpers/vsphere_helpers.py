...
class VSPHEREHELPERS(object):
    ...
    def generate_terraform_vars_for_scaleup(self, rhcos_ips):
        ...
        if self.folder_structure:
            ...
            scale_up_terraform_var_template_path_with_folder_structure = os.path.join(
                "ocp-deployment", scale_up_terraform_var_template_with_folder_structure
            )
            ...
            scale_up_terraform_var = convert_yaml2tfvars(scale_up_terraform_var_yaml)
        ...
        logger.info(f"scale-up terraform variable file: {scale_up_terraform_var}")
    ...
    def modify_scaleup_repo(self):
        ...
        if self.folder_structure:
            ...
            dict_data["vsphere"]["vcsa-qe"]["network"] = config.ENV_DATA["vm_network"]
            dict_data["vsphere"]["vcsa-qe"]["cpus"] = config.ENV_DATA["rhel_num_cpus"]
            dict_data["vsphere"]["vcsa-qe"]["memory"] = config.ENV_DATA["rhel_memory"]
            dict_data["vsphere"]["vcsa-qe"]["root_volume_size"] = config.ENV_DATA.get(
                "root_disk_size", "120"
            )
            ...
            replace_content_in_file(
                constants.SCALEUP_VSPHERE_ROUTE53,
                str_to_modify,
                f'"{config.DEPLOYMENT["aws_cred_path"]}"',
            )
    ...
    def generate_config_yaml(self):
        ...
        config.ENV_DATA["ssh_key_private"] = config.DEPLOYMENT["ssh_key_private"]
        scale_up_config_str = self._templating.render_template(
            scale_up_config_var_template_path, config.ENV_DATA
        )
        scale_config_var_yaml = os.path.join(sclaeup_data_config_dir, "config.yaml")
