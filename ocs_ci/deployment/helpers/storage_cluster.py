...
from ocs_ci.deployment.helpers.arbiter import get_arbiter_location
from ocs_ci.deployment.helpers.pgsql_secret import create_external_pgsql_secret
from ocs_ci.framework import config, merge_dict
from ocs_ci.framework.logger_helper import log_step
from ocs_ci.helpers import helpers
from ocs_ci.ocs import constants, defaults, ocp, pgsql
...
def create_storage_cluster(storageclass, fdf_deployment=False):
    """
    Create Storage Cluster.

    Args:
        storageclass (str): Storageclass to use.
        fdf_deployment (bool): Perform additional steps necessary for FDF deployments.

    Raises:
        UnsupportedFeatureError: When a feature is enabled for an incompatible OCS version.add()
    ...
    for key in config.COMPONENTS.keys():
        ...
        if config.COMPONENTS[key]:
            if "noobaa" in key:
                merge_dict(
                    cluster_data,
                    {"spec": {"multiCloudGateway": {"reconcileStrategy": "ignore"}}},
                )
            else:
                merge_dict(
                    cluster_data,
    ...
    cluster_data["metadata"]["namespace"] = config.ENV_DATA["cluster_namespace"]
    ...
    if storageclass:
        deviceset_data["dataPVCTemplate"]["spec"]["storageClassName"] = storageclass
    ...
    if fdf_deployment:
        cluster_data["metadata"]["labels"] = {"cns.isf.ibm.com/creator-fusion": ""}
        deviceset_data["name"] = constants.FDF_DEVICE_SET_NAME
    ...
    if config.ENV_DATA.get("custom_default_storageclass_names"):
        ...
        if cluster_data["spec"].get("nfs"):
            cluster_data["spec"]["nfs"] = {"storageClassName": storageclassnames["nfs"]}
    ...
    if set_managed_resources_ceph_cluster:
        ...
        if skip_upgrade_checks is not None:
            managed_resources_ceph_cluster["skipUpgradeChecks"] = skip_upgrade_checks
