...
def create_local_pvs():
    """
    Create one local PersistentVolume per worker node for agnostic DR deployments.

    Each PV is named ``local-pv-disk-<hostname>`` and is pinned to its node
    via nodeAffinity.  The disk path is discovered from
    ``/dev/disk/by-id/`` on each worker using the platform-specific pattern
    (wwn for vSphere, nvme for AWS, etc.).
    ...
    for worker in worker_names:
        logger.info("Discovering disk path on node %s", worker)
        ...
        disk_path = (
            "/dev/disk/by-id/"
            + [part for part in matching[0].split() if pattern in part][0]
        )
        logger.info("Node %s: using disk path %s", worker, disk_path)

        pv_data = templating.load_yaml(constants.LOCAL_PV_DISK_YAML)
        pv_data["metadata"]["name"] = f"local-pv-disk-{worker}"
        ...
        pv_yaml = tempfile.NamedTemporaryFile(
            mode="w+", prefix=f"local_pv_disk_{worker}_", delete=False
        )
        templating.dump_data_to_temp_yaml(pv_data, pv_yaml.name)
        logger.info("Creating PV local-pv-disk-%s", worker)
        run_cmd(f"oc apply -f {pv_yaml.name}")

    verify_pvs_created(
        expected_pvs=len(worker_names),
