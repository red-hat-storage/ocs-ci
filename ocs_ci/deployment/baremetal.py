...
from ocs_ci.ocs.node import get_nodes, get_node_internal_ip
...
from .helpers.lso_helpers import add_disk_for_vsphere_platform
...
def detect_simulation_disk_on_node(wnode, namespace="default", timeout=300):
    """
    Detects the last available /dev/sd*, dev/nvmen0* disk on a given worker node.
    ...
    Returns:
        str or None: The detected disk path (e.g., "/dev/sdb", "dev/nvmen0*") or None if not found.

    """
    logger.info(
        f"Attempting to auto-detect a suitable /dev/sd*, dev/nvmen0* "
...
def get_ceph_admin_key(host_node, namespace):
    """
...
def get_ceph_fsid(host_node, namespace):
    """
...
def remove_minimal_ceph_cluster(host_node, namespace="default"):
    """
...
def clear_ceph_bluestore_signature_on_wnodes(wnodes, disk_name=None, namespace="default"):
    """
...
def install_minimal_ceph_cluster(wnode, namespace="default"):
    """
...
def install_minimal_ceph_conf(wnode, host_node, namespace="default"):
    """
...
def install_minimal_ceph_cluster_and_conf_on_wnodes(
    wnodes, host_node=None, namespace="default"
):
    """
...
def simulate_ceph_bluestore_on_wnodes(wnodes, namespace="default"):
    """
...
def simulate_full_ceph_bluestore_process_on_wnodes(
    wnodes=None, namespace="default", add_disks=True, remove_ceph_cluster=True, clear_signatures=True
):
    """
    ...
    if add_disks and platform == constants.VSPHERE_PLATFORM:
        ...
        if disks_already_added:
            logger.info("Disks are already added to all worker nodes. Skipping addition.")
        else:
            add_disk_for_vsphere_platform()

    # Step 2: Install minimal Ceph cluster and configuration
    host_node = wnodes[0]
    result = install_minimal_ceph_cluster_and_conf_on_wnodes(wnodes, host_node, namespace)
    if not result:
    ...
    if clear_signatures:
        ...
        if not result:
            logger.warning("Failed to clear Ceph BlueStore signatures from worker nodes.")
            return False
