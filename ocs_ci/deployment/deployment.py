...
class Deployment(object):
    ...
    def deploy_with_external_mode(self):
        ...
        try:
            ...
            raise CephHealthExceptionExternal(
                f"External ceph cluster not healthy - {ex}"
            )
    ...
    def deploy_ocs(self):
        ...
        cluster_type = (
            constants.CLUSTER_TYPE_MULTI_SC
            if config.DEPLOYMENT.get("multi_storagecluster")
            else constants.CLUSTER_TYPE_INTERNAL
        )
        logger.info("Done creating rook resources, waiting for HEALTH_OK")
        try:
            ceph_health_check(
                namespace=self.namespace, tries=30, delay=10, cluster_type=cluster_type
            )
        except (CephHealthException, CephHealthExceptionInternal) as ex:
            err = str(ex)
        ...
        except CephHealthExceptionExternal as ex:
            logger.warning(f"Ceph not healthy for external cluster - {ex}")
            raise
