...
class FDFUpgrade(BaseUpgrade):
    ...
    def run_upgrade(self):
        """
        Execute the FDF upgrade procedure.

        This method orchestrates the FDF upgrade process by calling
        the upgrade method from FusionDataFoundationDeployment.
        ...
        if not self.fdf_deployment.live_upgrade:
    ...
    def update_subscription_channel(self):
        ...
        if len(version_parts) < 2:
            raise ConfigurationError(
                ...
                f"Expected format: 'vX.Y' or 'X.Y' (e.g., 'v4.21' or '4.21')"
            )

        upgrade_channel = f"stable-{version_parts[0]}.{version_parts[1]}"
        logger.info(
            f"Derived upgrade channel '{upgrade_channel}' from "
            f"fdf_upgrade_image_tag '{fdf_upgrade_image_tag}'"
        )

        logger.info(f"Waiting for upgrade channel '{upgrade_channel}' to be available")
        if not self.wait_for_subscription_channel(upgrade_channel, timeout=300):
            raise ChannelNotFound(
                f"Channel '{upgrade_channel}' did not become available within 300 seconds. "
                f"Cannot proceed with subscription channel update."
            )

        logger.info(f"Updating odf-operator subscription channel to: {upgrade_channel}")
        params_dict = {"spec": {"channel": upgrade_channel}}
    ...
    def monitor_fdf_upgrade(self, timeout=1800):
        """
    ...
    def _log_upgrade_failure_details(self):
        ...
        try:
            ...
            csvs_cmd = f"oc get csv -n {constants.OPENSHIFT_STORAGE_NAMESPACE} -o yaml"
        ...
        try:
            ...
            ip_cmd = (
                f"oc get installplan -n {constants.OPENSHIFT_STORAGE_NAMESPACE} -o yaml"
            )
    ...
    def wait_for_subscription_channel(self, channel_name, timeout=300):
        ...
        while time.time() - start_time < timeout:
            try:
                cmd = (
                    f"oc get packagemanifest odf-operator "
