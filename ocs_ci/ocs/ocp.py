...
def get_default_storage_class():
    ...
    Returns:
        list: StorageClass(es) list

    """
    count = 0
    default_storage_class = []
    default_sc_obj = OCP(kind='StorageClass')
    default_sc_objs = default_sc_obj.get()['items']
    for i in range(len(default_sc_objs)):
        if 'annotations' in default_sc_objs[i]['metadata']:
            ret = default_sc_objs[i]['metadata']['annotations'][
                'storageclass.kubernetes.io/is-default-class'
            ]
            if ret == 'true':
                count = count + 1
                default_storage_class.append(
                    default_sc_objs[i]['metadata']['name'])
    if count == 1:
        log.info("One default StorageClass found")
        return default_storage_class[0]
    elif count > 1:
        log.info("More than one default StorageClass found")
        return default_storage_class
    else:
        log.info("No default StorageClass found")
