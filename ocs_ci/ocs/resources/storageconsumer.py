...
class StorageConsumer:
    ...
    def set_storage_classes(self, storage_classes):
        ...
        with config.RunWithConfigContext(self.consumer_context):
            storage_classes_str = ", ".join(f'"{sc}"' for sc in storage_classes)
            patch_param = f'{{"spec": {{"storageClasses": [{storage_classes_str}]}}}}'
            self.ocp.patch(resource_name=self.name, params=patch_param)
...
def check_storage_classes_on_clients(ready_consumer_names: list[str]):
    ...
    def get_storage_classes():
        """
        Get the storage classes available on the provider.

        Returns:
            list[OCP]: List of storage class OCP objects available on the provider.
    ...
    with config.RunWithProviderConfigContextIfAvailable():
        storage_classes_on_provider = get_storage_classes()
    ...
    def wait_storage_classes_equal(given_classes):
        ...
        sample = TimeoutSampler(
            timeout=300,
            sleep=10,
            func=lambda: set(given_classes) == set(get_storage_classes()),
        )
    ...
    with ThreadPoolExecutor() as executor:
        futures = []
        for multicluster_config in config.get_consumer_indexes_list():
            if multicluster_config.current_cluster_name() in ready_consumer_names:
                futures.append(
                    executor.submit(
                        config_safe_thread_pool_task,
                        multicluster_config.index,
