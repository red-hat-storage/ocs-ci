...
def verify_device_class_in_osd_tree(
    ...
    for each in osd_tree["nodes"]:
        if each["type"] == "osd":
            osd_id = each.get("id")
            device_class = osd_id_per_deviceclass.get(osd_id, device_class)
            osd_name = each["name"]
...
def add_new_deviceset_in_storagecluster(
    device_class, name, count=3, replica=1, access_modes=None
):
    """
...
def get_deviceclass_name(device_set):
    """
    Get the deviceset name from the device set dict

    Args:
        device_set (dict): The device set dict

    Returns:
        str: the deviceset name
...
def get_deviceset_name(device_set):
    """
    Get the deviceset name from the device set dict
...
def get_deviceclass_name_per_count():
    """
    Get the deviceclass name per count dict

    Returns:
        dict: The deviceclass name per count dict

    """
    device_sets = get_all_device_sets()
    return {get_deviceclass_name(d): d["count"] for d in device_sets}
...
def get_osd_id_per_deviceclass():
    ...
    for d_name in deviceclass_names:
        ...
        for p in lvs_osd_pods:
            osd_id_per_deviceclass[get_osd_pod_id(p)] = d_name
...
def get_deviceset_name_per_deviceclass():
    """
    Get the deviceset name per deviceclass name dict

    Returns:
        dict: The deviceset name per deviceclass name dict

    """
    device_sets = get_all_device_sets()
    return {get_deviceset_name(d): get_deviceclass_name(d) for d in device_sets}
