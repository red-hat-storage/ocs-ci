...
def rwo_dynamic_pvc(
    ...
    logger.info(f"Deleting first pod so that second pod can attach PVC {pvc_obj.name}")
    pod_obj1.delete()
    ...
    pod_obj2.run_io(storage_type=storage_type, size="1G", fio_filename=pod_obj2.name)
...
def pvc_to_pvc_clone(pvc_factory, pod_factory, teardown_factory, interface_type):
    pvc_obj = pvc_factory(
    ...
    cloned_pvc_obj = pvc.create_pvc_clone(sc_name, parent_pvc, clone_yaml, namespace)
    ...
    logger.info(
        f"Checking the existence of {file_name} on cloned pod " f"{clone_pod_obj.name}"
    )
    assert pod.check_file_existence(
...
def pvc_snapshot(pvc_factory, pod_factory, teardown_factory, interface):
    pvc_obj = pvc_factory(interface=interface, size=5, status=constants.STATUS_BOUND)
    pod_obj = pod_factory(
