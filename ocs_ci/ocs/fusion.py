...
def create_fusion_monitoring_resources():
    ...
    logger.info(f"Creating {namespace_name} project")
    helpers.create_project(project_name=project_name)
    logger.info("Creating an OperatorGroup")
    og_path = os.path.join(constants.TEMPLATE_DIR, FUSION_TEMPLATE_DIR, "operatorgroup.yaml")
    og_data = templating.load_yaml(og_path)
    ...
    og_path = os.path.join(constants.TEMPLATE_DIR, FUSION_TEMPLATE_DIR, "subscription.yaml")
    og_data = templating.load_yaml(og_path)
    helpers.create_resource(**og_data)
...
def deploy_odf():
    ...
    offering_data["onboarding_validation_key"] = config.AUTH["managed_service"]["public_key"]
    template = _templating.render_template(
        "monitoringsecret.yaml.j2",
        secret_data,
    )
