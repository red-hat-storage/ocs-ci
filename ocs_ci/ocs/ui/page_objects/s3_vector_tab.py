...
class S3VectorTab(BucketsTab):
    ...
    def create_vector_bucket_via_s3api(
        ...
        ns_item_locator = (
            self.bucket_tab["s3_namespace_store_item"][0].format(namespace_store_name),
            By.XPATH,
        )
        self.do_click(ns_item_locator, enable_screenshot=True)
    ...
    def _select_bucketclass(self, bucketclass_name: str):
        ...
        # ResourceDropdown renders each item as a PatternFly MenuItem where the
        # inner <button> receives id="{resourceName}-link". Click by that id to
        # select the exact bucketclass and avoid false matches on a filtered list.
        exact_option_locator = (
            f"//button[@id='{bucketclass_name}-link']",
            By.XPATH,
        )
        self.do_click(exact_option_locator)
    ...
    def navigate_to_vector_bucket(self, displayed_name: str):
        ...
        bucket_locator = (
            self.s3_vector_loc["vector_bucket_link_by_name"][0].format(displayed_name),
            By.XPATH,
        )
        self.do_click(bucket_locator, enable_screenshot=True)
        self.page_has_loaded(sleep_time=self.PAGE_LOAD_WAIT)
    ...
    def _select_data_type(self, data_type: str):
        ...
        Bypasses do_click() entirely (which triggers page_has_loaded + copy_dom overhead
        totalling ~26 s when the element doesn't exist). Instead uses find_elements()
        with implicit wait temporarily set to 0 for an instant presence check.
        ...
        # Temporarily disable implicit wait for instant no-wait check.
        self.driver.implicitly_wait(0)
        try:
            elements = self.driver.find_elements(locator[1], locator[0])
        finally:
            self.driver.implicitly_wait(10)  # restore default
    ...
    def navigate_to_index(self, index_name: str):
        ...
        index_locator = (
            self.s3_vector_loc["index_link_by_name"][0].format(index_name),
            By.XPATH,
        )
        self.do_click(index_locator, enable_screenshot=True)
        self.page_has_loaded(sleep_time=self.PAGE_LOAD_WAIT)
