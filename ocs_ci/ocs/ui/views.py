...
VM_LOCATORS = {
    ...
    "actions_clone_option": ("//button[contains(., 'Clone')]", By.XPATH),
    # Clone VirtualMachine page
    "vm_kebab_menu": (
        "//button[@data-test-id='kebab-button' or contains(@aria-label, 'Actions')]",
        By.XPATH,
    ),
    "clone_vm_checkbox_start": (
        "//input[@type='checkbox' and (contains(@id, 'start-vm') or contains(@name, 'startVM'))]",
        By.XPATH,
    ),
    "clone_vm_checkbox_start_label": (
        "//label[contains(., 'Start VirtualMachine once created')]",
        By.XPATH,
    ),
    "clone_vm_button": (
        "//button[contains(., 'Clone') and (@type='submit' or contains(@class, 'pf-m-primary'))]",
        By.XPATH,
    ),
    "clone_modal": ("//div[contains(@class, 'pf-v5-c-modal-box')]", By.XPATH),
    # Console tab
    "console_tab": (
        "//button[contains(., 'Console')] | //a[contains(., 'Console')]",
        By.XPATH,
    ),
    "console_frame": ("//iframe[contains(@id, 'console')]", By.XPATH),
