...
def grafana_resource_consumption_ui(test_duration, url, username, password):
    ...
    try:
        ...
        time.sleep(3)
        ceph_cpu_xpath = '//h2[text()="Ceph Daemon CPU Usage"]/ancestor::div[contains(@class, "react-grid-item")]'
        ...
        menu_xpath = (
            '//button[@aria-label="Menu for panel with title Ceph Daemon CPU Usage"]'
        )
        ...
        panel_xp_2 = '//h2[text()="Ceph Daemon Memory Utilisation"]/ancestor::div[contains(@class, "react-grid-item")]'

        panel_container_2 = wait.until(
            ec.presence_of_element_located((By.XPATH, panel_xp_2))
        )
        ActionChains(driver).move_to_element(panel_container_2).perform()

        menu_xpath_2 = '//button[@aria-label="Menu for panel with title Ceph Daemon Memory Utilisation"]'
