...
class WorkloadScalingHelper:
    ...
    def wait_for_scaling_completion(self, scaling_thread, timeout=120):
        ...
        Returns:
            bool: True if thread completed, False if timeout
        """
        if scaling_thread and scaling_thread.is_alive():
            log.info(
                f"Waiting for scaling operations to complete (timeout: {timeout}s)"
            )
