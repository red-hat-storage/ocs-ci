...
def exec_ceph_command(toolbox_pod, command, timeout=300):
    """
    Execute a Ceph command in the toolbox pod and return its output.

    Args:
        toolbox_pod (Pod): Ceph toolbox pod (rook-ceph-tools)
        command (str): Ceph command to execute (e.g. ``ceph auth ls``)
        timeout (int): Command timeout in seconds

    Returns:
        str: Command output
    """
    return toolbox_pod.exec_cmd_on_pod(command, out_yaml_format=False, timeout=timeout)
