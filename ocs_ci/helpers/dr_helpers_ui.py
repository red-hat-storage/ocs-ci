...
def verify_app_in_protected_applications_list(
    ...
    # Check if app is present in the list
    # Primary locator: uses data-test="resource-link-{app_name}" attribute
    app_locator = format_locator(
        acm_loc["protected-app-name-in-list"], app_name
    )
    log.info(f"Looking for app with locator: {app_locator}")
...
def verify_protected_applications_list_view(
    ...
    for app_type, app_name in apps_to_verify:
        ...
        # Clear search for next application
        try:
            import platform
            from selenium.webdriver.common.keys import Keys

            acm_loc = locators_for_current_ocp_version()["acm_page"]
            search_element = acm_obj.driver.find_element(
                acm_loc["protected-app-search-bar"][1],
                acm_loc["protected-app-search-bar"][0],
            )
            # Use COMMAND on Mac, CONTROL on Windows/Linux
            modifier_key = Keys.COMMAND if platform.system() == "Darwin" else Keys.CONTROL
            search_element.send_keys(modifier_key + "a")  # Select all
            search_element.send_keys(Keys.DELETE)  # Delete
            time.sleep(1)
        except Exception as e:
            log.debug(f"Could not clear search bar: {e}")
...
def verify_protected_app_kebab_menu_actions(
    ...
    # Use keyboard shortcut to select all and delete (more reliable than do_clear)
    import platform
    from selenium.webdriver.common.keys import Keys

    search_element = acm_obj.driver.find_element(
        acm_loc["protected-app-search-bar"][1], acm_loc["protected-app-search-bar"][0]
    )
    # Use COMMAND on Mac, CONTROL on Windows/Linux (for Jenkins compatibility)
    modifier_key = Keys.COMMAND if platform.system() == "Darwin" else Keys.CONTROL
    search_element.send_keys(modifier_key + "a")  # Select all
    search_element.send_keys(Keys.DELETE)  # Delete selected text
    time.sleep(1)

    # Now enter the app name
    acm_obj.do_send_keys(acm_loc["protected-app-search-bar"], text=app_name)
    app_locator = format_locator(acm_loc["protected-app-name-in-list"], app_name)
    acm_obj.wait_for_element_to_be_visible(app_locator, timeout=30)
