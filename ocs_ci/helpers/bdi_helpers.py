...
def install_db2u(class_instance):
    ...
    os.chdir(class_instance.bdi_dir + class_instance.chart_dir + "/common")

    with open("helm_options", "r") as file:
        data = file.read()
        data = data.replace("40Gi", class_instance.pvc_size)
        with open(class_instance.temp_helm_options.name, "w") as f:
            f.write(data)

    log.info("Installing db2warehouse")
    install_cmd = (
        f"./db2u-install --db-type db2wh --namespace {class_instance.db2u_project} "
        f"--release-name {class_instance.db2u_r_n} --helm-opt-file "
    ...
    pod = ocp.OCP(kind=constants.POD, namespace=class_instance.db2u_project)
    assert pod.wait_for_resource(
        condition="Running",
        selector="app=" + class_instance.db2u_r_n,
        resource_count=6,
        timeout=600,
    )
...
def clean_temp_files(class_instance):
    ...
    if os.path.isfile(class_instance.temp_helm_options.name):
        run_cmd(cmd=f"rm -f {class_instance.temp_helm_options.name}")

    if os.path.isfile(class_instance.temp_yaml_configure.name):
        run_cmd(cmd=f"rm -f {class_instance.temp_yaml_configure.name}")

    if os.path.isfile(class_instance.temp_yaml_data.name):
        run_cmd(cmd=f"rm -f {class_instance.temp_yaml_data.name}")

    if os.path.isfile(class_instance.temp_yaml_run.name):
        run_cmd(cmd=f"rm -f {class_instance.temp_yaml_run.name}")
    run_cmd(cmd="rm -f tmp*")
