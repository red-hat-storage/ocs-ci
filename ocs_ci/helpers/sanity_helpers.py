...
class Sanity:
    ...
    def __init__(self):
        ...
        self.obc_objs = list()
        self.obj_data = ""
        if not is_storage_client_cluster_cluster():
    ...
    def health_check(self, cluster_check=True, tries=20):
        ...
        if not (
            ...
            or is_storage_client_cluster_cluster()
        ):
            ceph_health_check(
...
class SanityProviderMode(Sanity):
    ...
    def __init__(
        ...
        This method uses the 'create_scale_pods_and_pvcs_using_kube_job_on_storage_client_clusters' factory.

        Args:
           create_scale_pods_and_pvcs_using_kube_job_on_storage_client_clusters (function): Factory for creating scale
        ...
        self.create_resources_on_storage_client_clusters_factory = (
            create_scale_pods_and_pvcs_using_kube_job_on_storage_client_clusters
        )
        self.scale_count = scale_count
        self.pvc_per_pod_count = pvc_per_pod_count
        ...
        if client_indices:
            ...
            if is_storage_client_cluster_cluster():
                logger.info(
    ...
    def create_resources_on_clients(self, tries=1, delay=30):
        ...
        using the method 'base_create_resources_on_storage_client_clusters'. If not specified, the default value of
        'tries' is 1 - which means that by default, it only tries to create the resources once.
        In every iteration, if it fails to generate the resources, it cleans up the current resources
        before continuing to the next iteration.
    ...
    def base_create_resources_on_clients(self):
        ...
        This function uses the factory "create_scale_pods_and_pvcs_using_kube_job_on_storage_client_clusters"
