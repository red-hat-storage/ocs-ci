...
class CephXKeyRotation:
    ...
    def get_spec_key_type(self):
        """Return ``spec.security.cephx.keyType`` from the CephCluster."""
        return self.get_spec_cephx().get("keyType")

    def patch_cephcluster_key_type(self, key_type):
        """Set ``spec.security.cephx.keyType`` on the CephCluster."""
        cluster = self._get_cluster_dict()
        ...
        elif "keyType" in cephx:
            patch_ops.append(
                {
                    "op": "replace",
                    "path": "/spec/security/cephx/keyType",
                    "value": key_type,
                }
            )
        else:
        ...
        log.info(f"Patching CephCluster spec.security.cephx.keyType to {key_type}")
    ...
    def remove_cephcluster_key_type(self):
        """Remove ``spec.security.cephx.keyType`` from the CephCluster if present."""
        cephx = self.get_spec_cephx()
        if "keyType" not in cephx:
            log.info(
                "CephCluster spec.security.cephx.keyType not set; nothing to remove"
            )
            return

        log.info("Removing CephCluster spec.security.cephx.keyType")
        self.cephcluster_obj.patch(
            params=json.dumps(
                [{"op": "remove", "path": "/spec/security/cephx/keyType"}]
            ),
            format_type="json",
        )
        self._reload()

    def wait_for_cephcluster_key_type(self, key_type, timeout=300, sleep=10):
        """Wait until CephCluster ``spec.security.cephx.keyType`` matches *key_type*."""
        log.info(f"Waiting for CephCluster keyType={key_type}")

        def _matches():
            actual = self.get_spec_key_type()
            if actual == key_type:
                return True
            log.debug(f"CephCluster keyType={actual}, want {key_type}")
            return False

        for matched in TimeoutSampler(timeout, sleep, _matches):
            if matched:
                log.info(f"CephCluster keyType is {key_type}")
                return True

        raise UnexpectedBehaviour(
            f"Timed out waiting for CephCluster keyType={key_type}"
        )
