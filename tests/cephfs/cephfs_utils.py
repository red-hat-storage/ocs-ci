    ...
    def clean_up(self,fuse_clients,kernel_clients,mounting_dir,umount=None):
       try:
            for client in fuse_clients:
                ...
                client.exec_command(cmd='sudo rm -rf %s*' %(mounting_dir))
                if umount != None:
                    log.info("Unmounting fuse client:")
                    client.exec_command(cmd='sudo fusermount -u %s' %(mounting_dir))
            ...
            for client in kernel_clients:
                ...
                if umount != None:
                    client.exec_command(cmd='sudo umount %s'%(mounting_dir))
                    client.exec_command(cmd='sudo rmdir %s' %(mounting_dir))
                break
