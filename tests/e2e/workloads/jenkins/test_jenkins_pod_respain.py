...
class TestJenkinsPodRespin(E2ETest):
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            pytest.param(
                *['mon'], marks=pytest.mark.polarion_id("OCS-2204")
            ),
            pytest.param(
                *['osd'], marks=pytest.mark.polarion_id("OCS-2179")
            ),
            pytest.param(
                *['mgr'], marks=pytest.mark.polarion_id("OCS-2205")
            ),
        ]
    )
    @pytest.mark.usefixtures(jenkins_setup.__name__)
    def test_run_jenkins_respin_pod(self, jenkins, pod_name):
        """
