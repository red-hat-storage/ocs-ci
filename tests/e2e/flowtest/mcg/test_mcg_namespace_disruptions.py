...
from ocs_ci.framework.pytest_customization.marks import skipif_aws_creds_are_missing, flowtests
...
class TestMcgNamespaceDisruptions(E2ETest):
    ...
    def test_mcg_namespace_disruptions(
        ...
        logger.info(f'Verifying whether user: {user.email_id} has only public read access')
        try:
            s3_put_object(
                s3_obj=user, bucketname=ns_bucket,
            ...
            assert False, "Put object operation was granted access, when it should have denied"
