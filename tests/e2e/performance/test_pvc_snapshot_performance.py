    ...
    def test_pvc_snapshot_performance(self, teardown_factory, pvc_size):
        ...
        for test_num in range(self.tests_numbers):
            ...
            restore_pvc_obj = pvc.create_restore_pvc(
                sc_name=sc_name, snap_name=self.snap_obj.name,
                namespace=self.snap_obj.namespace, size=pvc_size,
        ...
        log.info(f" Average snapshot creation time is {c_runtime / self.tests_numbers} sec.")
        log.info(f" Average snapshot creation speed is {c_speed / self.tests_numbers} MiB/sec")
        log.info(f" Average snapshot restore time is {r_runtime / self.tests_numbers} sec.")
        log.info(f" Average snapshot restore speed is {r_speed / self.tests_numbers} MiB/sec")

    @pytest.mark.parametrize(
        argnames=["file_size", "files", "threads", "interface"],
        argvalues=[
            pytest.param(*[32, 125000, 8, constants.CEPHBLOCKPOOL],),
            pytest.param(*[16, 250000, 8, constants.CEPHBLOCKPOOL],),
            pytest.param(*[8, 500000, 8, constants.CEPHBLOCKPOOL],),
            pytest.param(*[32, 125000, 8, constants.CEPHFILESYSTEM],),
            pytest.param(*[16, 250000, 8, constants.CEPHFILESYSTEM],),
            pytest.param(*[8, 500000, 8, constants.CEPHFILESYSTEM],),
        ]
    )
