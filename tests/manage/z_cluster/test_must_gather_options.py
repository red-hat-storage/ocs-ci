...
class TestMustGatherTimeoutOptions(ManageTest):
    ...
    @pytest.mark.parametrize(
        argnames=["expected_result", "cmd_param", "timeout"],
        argvalues=[
            pytest.param(*["Pass", "request-timeout", "1200s"]),
            pytest.param(*["Pass", "request-timeout", "20m"]),
            pytest.param(*["Pass", "request-timeout", "1h"]),
            pytest.param(*["Pass", "timeout", "1200"]),
            pytest.param(*["Fail", "timeout", "-1200"]),
            pytest.param(*["Fail", "timeout", "-1200s"]),
            pytest.param(*["Fail", "timeout", "-20m"]),
            pytest.param(*["Fail", "timeout", "-1h"]),
        ],
    )
    @pytest.mark.skipif(
    ...
    def test_must_gather_timeout(self, expected_result, cmd_param, timeout):
        """
        Test must-gather timeout parameter
        """
        logger.info(
            f"Running must-gather command with '{cmd_param}={timeout}'. Operation should {expected_result}."
        )
        ...
        # create_directory_path(self.temp_folder)
        cmd = f"adm must-gather --image={image} --dest-dir={self.temp_folder} --{cmd_param}={timeout}"

        occli = OCP()
        try:
            occli.exec_oc_cmd(cmd, out_yaml_format=False, timeout=1200)
        except CommandFailed as ex:
            # assertion when the must-gather failed but should pass
            assert (
                not expected_result == "Pass"
            ), f"Must gather failed when it should pass! Error: {ex}"
            # must-gather failed as expected
            logger.info(f"Failed during must gather as expected. Error: {ex}")
        except TimeoutExpired as ex:
            logger.error(
                f"Timeout {timeout}s for must-gather reached, command"
