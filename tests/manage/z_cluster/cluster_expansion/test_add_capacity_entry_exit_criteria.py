...
class TestAddCapacity(ManageTest):
    def test_add_capacity(
        self, project_factory, multi_dc_pod, multi_pvc_factory,
        ...
        if config.ENV_DATA['platform'].lower() == constants.AWS_PLATFORM:
            ...
            if aws_number_of_zones == 1:
                assert cluster_helpers.check_osd_tree_1az_aws(tree_output, len(osd_pods_after)), \
                    "Exit criteria verification FAILED: Incorrect ceph osd tree formation found"
            else:
                assert cluster_helpers.check_osd_tree_3az_aws(tree_output, len(osd_pods_after)), \
                    "Exit criteria verification FAILED: Incorrect ceph osd tree formation found"
        ...
        assert cluster_obj.get_ceph_health() != 'HEALTH_ERR', 'Ceph cluster health checking failed'
