...
def setup_fs(self):
    ...
    """
    global CEPH_OBJ
    self.fs_data = templating.get_crd_dict(defaults.CEPHFILESYSTEM_YAML)
...
class TestOSCBasics(ManageTest):
    ...
    def test_ocs_336(self, test_fixture):
        ...
        """
        self.cephfs_secret = templating.get_crd_dict(
            defaults.CSI_CEPHFS_SECRET
        )
        ...
        secret.create()
        self.cephfs_sc = templating.get_crd_dict(
        ...
        self.cephfs_pvc = templating.get_crd_dict(defaults.CSI_CEPHFS_PVC)
    ...
    def test_ocs_346(self):
        ...
        """
        self.rbd_secret = templating.get_crd_dict(defaults.CSI_RBD_SECRET)
        ...
        secret.create()
        self.rbd_sc = templating.get_crd_dict(
            defaults.CSI_RBD_STORAGECLASS_DICT
        )
        self.rbd_sc['parameters']['monitors'] = self.mons
        del self.rbd_sc['parameters']['userid']
        storage_class = OCS(**self.rbd_sc)
        storage_class.create()
        self.rbd_pvc = templating.get_crd_dict(defaults.CSI_RBD_PVC)
