...
@pytest.mark.parametrize(
    ...
    argvalues=[
        pytest.param(
            constants.CEPHBLOCKPOOL, marks=pytest.mark.polarion_id("OCS-00000")
        ),
        pytest.param(
            constants.CEPHFILESYSTEM, marks=pytest.mark.polarion_id("OCS-0000")
        )
    ]
)
class TestPvcResize(ManageTest):
    ...
    def test_setup(self, interface, storageclass_factory, pvc_factory, pod_factory):
        ...
        self.pvc_obj = pvc_factory(
            interface=interface, storageclass=self.sc_obj, size=10
        )
    ...
    def test_pvc_resize(self):
        ...
        # Wait for 60 seconds to reflect the change on pod
        for df_out in TimeoutSampler(
            60, 3, self.pod_obj.exec_cmd_on_pod, command='df -kh'
        ):
            df_out = df_out.split()
            new_size_mount = df_out[df_out.index(self.pod_obj.get_storage_path()) - 4]
            if new_size_mount == f'{pvc_size_new}G':
                break
            log.info(
