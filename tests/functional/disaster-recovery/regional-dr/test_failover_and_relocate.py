...
class TestFailoverAndRelocate:
    ...
    def test_failover_and_relocate(
        ...
        drpc_subscription = DRPC(namespace=rdr_workloads[0].workload_namespace)
        drpc_appset = DRPC(
            namespace=constants.GITOPS_CLUSTER_NAMESPACE,
            resource_name=f"{rdr_workloads[1].appset_placement_name}-drpc",
        )
        ...
        for rdr_workload in rdr_workloads:
            if config.RUN.get("rdr_failover_via_ui"):
                ...
                dr_helpers.failover(
                    ...
                    rdr_workload.workload_type,
                    rdr_workload.appset_placement_name
                    if rdr_workload.workload_type != constants.SUBSCRIPTION
                    else None,
                )
