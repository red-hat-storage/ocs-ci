...
Required config parameters in multicluster_mode_rdr.yaml:
    ...
    ENV_DATA:
        # Skip flags - set to true to skip specific deployment stages
        skip_acm_deployment: false           # Skip ACM Hub deployment
        skip_mce_deployment: false           # Skip MCE deployment
        skip_acm_import: false               # Skip ACM cluster import
        skip_submariner_deployment: false    # Skip Submariner deployment
        skip_gitops_deployment: false        # Skip GitOps deployment
        skip_oadp_deployment: false          # Skip OADP deployment
...
def test_deploy_rdr():
    ...
    # Check skip flag
    if config.ENV_DATA.get("skip_mce_deployment", False):
        log.info("MCE deployment is disabled (skip_mce_deployment: true)")
    ...
    # Check skip flag
    if config.ENV_DATA.get("skip_acm_import", False):
        log.info("ACM cluster import is disabled (skip_acm_import: true)")
        log.info("Skipping ACM cluster import")
    elif acm_cluster_index is not None:
        ...
        if version_util.compare_versions(
            ...
            def apply_idms(cluster):
                ...
                Returns:
                    str: 'applied' if newly created, 'existing' if already present.
                """
                cluster_index = cluster.MULTICLUSTER.get("multicluster_index")
                cluster_name = cluster.ENV_DATA.get(
                    "cluster_name", f"cluster-{cluster_index}"
                ...
                try:
                    ...
                    if idms_obj.check_resource_existence(
                        ...
                        log.info(
                            f"[{cluster_name}] ImageDigestMirrorSet 'acm-idms' already present,"
                            " will verify MCP readiness"
                        )
                        return "existing"
                    log.info(
            ...
            acm_hub_unreleased = config.clusters[acm_cluster_index].ENV_DATA.get(
                "acm_hub_unreleased"
            )
            ...
            for cluster in config.clusters:
                if (
                    cluster.DEPLOYMENT.get("disconnected", False)
                    or not acm_hub_unreleased
                ):
                    log.info(
            ...
            # Verify MCP readiness on all clusters where IDMS is present
            log.info("-" * 80)
            log.info(
                "Waiting for MachineConfigPool updates on clusters where IDMS is present..."
            )
    ...
    if config.ENV_DATA.get("skip_dr_deployment", False):
        log.info("RDR deployment is disabled (skip_dr_deployment: true)")
        log.info("Skipping RDR deployment")
    else:
    ...
    # Check skip flag
    if config.ENV_DATA.get("skip_post_deployment_validation", False):
        log.info(
            "Post-deployment validation is disabled (skip_post_deployment_validation: true)"
        )
        log.info("Skipping post-deployment validation")
        validation_results = []
    else:
