...
class TestCorsConfig:
    ...
    def test_basic_cors_operations(
        ...
        curl_output = self.exec_curl_command(mcg_obj.s3_endpoint, bucket_name, allowed_origin, http_method)
        """
        curl_command = (
        ...
        incorrect_origin = choice(cors_config['CORSRules'][0]['AllowedOrigins']) + ".incorrect.origin"
        http_method = choice(cors_config['CORSRules'][0]['AllowedMethods'])
        extra_header= {'Content-Type': 'application/json'}
        curl_output = self.exec_curl_command(mcg_obj.s3_endpoint, bucket_name, incorrect_origin, http_method, extra_header)
        """
        curl_command = (
            f"curl -k {mcg_obj.s3_endpoint}/{bucket_name} -X OPTIONS "
            f"-H 'Access-Control-Request-Method: {choice(cors_config['CORSRules'][0]['AllowedMethods'])}' "
            f"-H 'Origin: {choice(cors_config['CORSRules'][0]['AllowedOrigins'])}.incorrect.origin' "
            f"-H 'Content-Type: application/json' -D -"
        )
        curl_output = exec_cmd(cmd=curl_command).stdout.decode()
        logger.info(curl_output)
