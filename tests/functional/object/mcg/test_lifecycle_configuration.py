...
class TestLifecycleConfiguration(MCGTest):
    ...
    def test_expired_object_delete_marker(self, mcg_obj, bucket_factory, awscli_pod):
        ...
        # TODO optional?
        # 8. Verify that the object is no longer listed either in versions listing or standard
        objects_listed = list_objects_from_bucket(
            pod_obj=awscli_pod,
            target=bucket,
            s3_obj=mcg_obj,
        )
        assert len(objects_listed) == 0, "Object was not deleted as expected"

        versions_listed = s3_list_object_versions(
            mcg_obj,
            bucket,
        )
        assert len(versions_listed) == 0, "Object was not deleted as expected"
