...
@mcg
@red_squad
class TestS3VectorWithFactory(MCGTest):
    """
    Test S3 Vector functionality against noobaa
    """

    @tier2
    def test_s3_vector_operations_with_factory(self, vector_bucket_factory, mcg_obj):
        """
        ...
        # Step 2: Validate vector bucket exists and update mcg_obj withOBC creds
        logger.info("Validating vector bucket exists and logging OBC credentials")
        obc_obj = OBC(vector_bucket_name)
        mcg_obj.access_key = obc_obj.access_key
        mcg_obj.access_key_id = obc_obj.access_key_id

        bucket_info = vector_utils.get_vector_bucket(mcg_obj, vector_bucket_name)
        assert (
        ...
        indices_response = vector_utils.list_indexes(
            mcg_obj, vectorBucketName=vector_bucket_name
        )
        ...
        deleted_count = vector_utils.delete_all_vectors(
            mcg_obj, vectorBucketName=vector_bucket_name, indexName=index_name
        )
        ...
        vectors_after_delete = vector_utils.list_vectors(
            mcg_obj, vectorBucketName=vector_bucket_name, indexName=index_name
        )
        remaining = vectors_after_delete.get("vectors", [])
        assert (
            len(remaining) == 0
        ), f"Vectors still exist after deletion: {len(remaining)}"
        ...
        vector_utils.delete_vector_bucket(mcg_obj, vector_bucket_name)
        logger.info(f"✓ Vector bucket '{vector_bucket_name}' deleted")

        # Verify bucket is deleted
        buckets_after_delete = vector_utils.list_vector_buckets(mcg_obj)
        remaining_buckets = [
