...
class TestVmOperationsUseClusterCapacity(E2ETest):
    """
    Tests for VM operations with VM count based on cluster capacity.
    """

    def test_vm_lifecycle_and_io_use_cluster_capacity(
        self, setup_cnv, project_factory, multi_cnv_workload
    ):
        """
        This test performs the VM lifecycle operations and IO with VM count
        calculated from available cluster capacity (CPU, RAM, storage).
        ...
        1) Create a VM using a standalone PVC/DV/DVT
           ...
           VM count is determined by cluster capacity when use_cluster_capacity=True.
