...
class TestRbdBlockPvc(ManageTest):
    ...
    def test_rbd_block_rwx_pvc(self, pod_factory):
        ...
        """
        logger.test_step("Find initial md5sum value from all pods")
        ...
        logger.assertion(
            f"Initial md5sum values from pods: expected unique count=1, "
            f"actual unique count={len(set(md5sum_values_initial))}"
        )
        assert (
        ...
        logger.test_step("Run IO from each pod and verify md5sum changes on all pods")
        for io_pod in self.pod_objs:
            logger.info(f"Running IO from pod {io_pod.name}")
            io_pod.run_io(
                storage_type="block",
            ...
            logger.debug(f"IO started on pod {io_pod.name}")

            # Wait for IO completion
            io_pod.get_fio_results()
            logger.debug(f"IO completed on pod {io_pod.name}")
        ...
        logger.test_step("Delete all pods and create a new pod using the same PVC")
        logger.info("Deleting the pods")
        for pod_obj in self.pod_objs:
            pod_obj.delete()
        ...
        logger.test_step("Verify md5sum from new pod matches last known value")
        logger.info("Finding md5sum value from new pod")
        ...
        logger.assertion(
            f"md5sum from new pod: expected='{md5sum_value_initial}', actual='{md5sum_new}'"
        )
        assert (
            md5sum_new == md5sum_value_initial
        ), f"md5sum mismatch on new pod. Expected {md5sum_value_initial}. Obtained {md5sum_new}"

        logger.test_step("Run IO from new pod and verify completion")
        logger.info("Running IO from new pod")
        pod_obj_new.run_io(
            storage_type="block",
            size=f"{int(self.pvc_size/2)}M",
            io_direction="write",
        ...
        logger.info(f"IO started on the new pod {pod_obj_new.name}")

        # Wait for IO completion
        pod_obj_new.get_fio_results()
        logger.info(f"IO completed on the new pod {pod_obj_new.name}")
