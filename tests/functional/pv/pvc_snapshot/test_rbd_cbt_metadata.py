...
class TestRbdCBTMetadata(ManageTest):
    ...
    def setup(
        ...
        PVCs and pods created through factories are cleaned up
        by their respective factory finalizers. The CBT runner
        is cleaned up via the teardown fixture.
        """
    ...
    def teardown(self, request):
        """Clean up CBT runner resources after each test."""
    ...
    def test_cbt_delta_incremental(self, volume_mode):
        ...
        # Clean up the restored PVC so the second verifier pod
        # can mount copy-pvc exclusively.
        restored_pvc_1.delete()
        restored_pvc_1.ocp.wait_for_delete(restored_pvc_1.name)
