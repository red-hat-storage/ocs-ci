...
class TestVolumeGroupSnapshot(ManageTest):
    ...
    def setup(
        ...
        for i in range(PVC_COUNT):
            ...
            helpers.wait_for_resource_state(pvc_obj, constants.STATUS_BOUND, timeout=90)
            pvc_obj.reload()
        ...
        for pvc_obj in self.pvc_objs:
            ...
            pod_obj.run_io(storage_type="fs", size="100M", fio_filename=file_name)
    ...
    def test_volume_group_snapshot(self, interface, teardown_factory):
        ...
        log.info("Waiting for individual VolumeSnapshots to be deleted")
        for snap in snapshots:
            snap_ocp.wait_for_delete(
                resource_name=snap["metadata"]["name"], timeout=180
            )

        log.info("Verifying VolumeSnapshots are cleaned up")
        remaining_snaps = snap_ocp.get().get("items", [])
        remaining_snap_names = [
            snap.get("metadata", {}).get("name") for snap in remaining_snaps
        ]
        assert len(remaining_snaps) == 0, (
            f"Expected 0 VolumeSnapshots after VGS deletion, found "
            f"{len(remaining_snaps)}: {remaining_snap_names}"
        )
