...
class TestPvcMultipleSnapshot(ManageTest):
    """
    Tests to verify more than 256 snapshots --> restore --> snapshot --> restore
    """
    ...
    def setup(self, pvc_factory, pod_factory):
        ...
        """
        self.pvc_obj = pvc_factory(
            interface=constants.CEPHBLOCKPOOL, size=1, status=constants.STATUS_BOUND
        )
        self.pod_obj = pod_factory(
    ...
    @run_on_all_clients_push_missing_configs
    def test_pvc_multiple_snapshots(self, teardown_factory, cluster_index):
        """

        Test multiple snapshots from the same PVC
        ...
        interface = constants.CEPHBLOCKPOOL
        log.info(f"Running IO on pod {self.pod_obj.name}")
        file_name = "test_file_fio"
        log.info(f"File created during IO {file_name}")
        self.pod_obj.run_io(
            storage_type="fs", size="1M", fio_filename=file_name, runtime=5
        )
        ...
        if interface == constants.CEPHFILESYSTEM:
            snap_yaml = constants.CSI_CEPHFS_SNAPSHOT_YAML
        for num_snapshots in range(1, 301):
            log.info(
                f"Starting iteration {num_snapshots} of creating and restoring snapshot of the PVC"
            )
            snap_name = helpers.create_unique_resource_name("test", "snapshot")
            snap_obj = pvc.create_pvc_snapshot(
                self.pvc_obj.name,
            ...
            log.info("Data integrity check passed, md5sum are same")
            log.info(
                f"End of iteration {num_snapshots} of creating and restoring snapshot of the PVC"
            )

    @run_on_all_clients_push_missing_configs
    def test_pvc_chain_of_snapshot_restore(self, teardown_factory, cluster_index):
        """
        Test chain of snapshot restore operations

        Do multiple iterations of the steps given below
        1. Run I/O on a pod file.
        2. Calculate md5sum of the file.
        3. Take a snapshot of the PVC.
        4. Create a new PVC out of that snapshot.
        ...
        7. Verify that the md5sum of the file on the new pod matches
           with the md5sum of the file on the original pod.
        8. Create snapshot of the PVC and attach it to pod
        ...
        interface = constants.CEPHBLOCKPOOL
        for num_snapshots in range(1, 301):
            log.info(f"Starting snapshot-restore iteration {num_snapshots}")
            log.info(f"Running IO on pod {self.pod_obj.name}")
            file_name = "test_file_fio"
            log.info(f"File created during IO {file_name}")
            self.pod_obj.run_io(storage_type="fs", size="1M", fio_filename=file_name)
            ...
            log.info(f"Ending snapshot-restore iteration {num_snapshots}")
            self.pod_obj = restore_pod_obj
            self.pvc_obj = restore_pvc_obj
