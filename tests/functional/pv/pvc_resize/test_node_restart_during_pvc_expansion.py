...
class TestNodeRestartDuringPvcExpansion(ManageTest):
    ...
    def teardown(self, request, nodes):
        ...
        def finalizer():
            ...
            logger.info("Verify that we can execute a Ceph command successfully")
            ...
            if not ceph_cmd_success and is_ms_consumer_cluster():
                ...
                logger.info("Try to restart the ocs-operator pod")
    ...
    def test_worker_node_restart_during_pvc_expansion(self, nodes):
        ...
        logger.test_step(
            f"Restart worker node {selected_node[0].name} and expand PVCs concurrently"
        )
        # Restart node
        logger.info(f"Restarting node {selected_node[0].name}")
        restart_thread = executor.submit(nodes.restart_nodes, nodes=selected_node)

        logger.info(f"Expanding all {len(self.pvcs)} PVCs to {pvc_size_expanded}G")
        ...
        logger.test_step("Verify node is ready and PVC expansion succeeded")
        ...
        logger.assertion(
            f"Pod count after reboot: expected={len(self.pods)}, actual={len(new_pods_list)}"
        )
        ...
        for pvc_obj in self.pvcs:
            logger.assertion(
                f"PVC {pvc_obj.name} expansion result: expected=True, actual={pvc_obj.expand_proc.result()}"
            )
            ...
            logger.assertion(
                f"PVC {pvc_obj.name} capacity: expected='{pvc_size_expanded}Gi', actual='{capacity}'"
            )
        ...
        logger.test_step("Verify expanded size is reflected on pods")
        ...
        logger.test_step("Run IO after PVC expansion and verify completion")
        logger.info(f"Running IO on {len(new_pods_list)} pods after PVC expansion")
        for pod_obj in new_pods_list:
        ...
        logger.assertion(
            "Ceph command execution: expected=True,"
            " actual checked via wait_for_ceph_cmd_execute_successfully()"
        )
        assert (
            wait_for_ceph_cmd_execute_successfully()
        ...
        for pod_obj in new_pods_list:
            ...
            logger.assertion(
                f"IO error count on pod {pod_obj.name}: expected=0, actual={err_count}"
            )
            assert err_count == 0, (
