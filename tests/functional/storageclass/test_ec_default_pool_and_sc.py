...
@green_squad
class TestDefaultSCReconciliation(ManageTest):
    """
    Verify ODF operator reconciles the default StorageClass annotation
    after manual removal. Applies to any ODF cluster.
    """

    @tier2
    @pytest.mark.polarion_id("OCS-8033")
    def test_default_sc_reconciliation(self, restore_default_sc_annotation):
        """
        Steps:
        1. Verify the default RBD StorageClass has the is-default-class annotation
        2. Remove the default annotation
        3. Verify SC is no longer default
        4. Wait for operator reconciliation (up to 300s)
        5. Verify the annotation is restored
        """
        rbd_sc_name = constants.DEFAULT_STORAGECLASS_RBD
