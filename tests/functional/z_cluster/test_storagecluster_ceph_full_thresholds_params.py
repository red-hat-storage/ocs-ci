...
def thresholds_teardown_fixture(self, request):
    ...
    def finalizer():
        configure_cephcluster_params_in_storagecluster_cr(
            params=self.thresholds, default_values=True
        )
...
class TestStorageClusterCephFullThresholdsParams(ManageTest):
    ...
    def setup_thresholds_params(self):
        configure_cephcluster_params_in_storagecluster_cr(
            params=self.thresholds, default_values=False
        )
    ...
    def verify_thresholds_params(self):
        ...
        for parameter in self.thresholds:
        ...
        sample = TimeoutSampler(
            ...
            cmd="ceph osd dump",
            expected_output_lst=(
                tuple(f"{d['ceph_key']} {d['value']}" for d in self.thresholds)
            ),
            cephtool_cmd=True,
