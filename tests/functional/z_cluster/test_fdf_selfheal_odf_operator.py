"""
FDF Standalone — ODF Operator Crash Recovery (RHSTOR-8290 / OCS-8193).

Delete ODF operator controller-manager pod and verify it restarts,
StorageCluster reconciles, and workload I/O is uninterrupted.
"""
...
class TestFDFSelfHealODFOperator:
    """
    Verify ODF operator self-heals from pod crash on FDF standalone.
    """
    ...
    def test_odf_operator_crash_recovery(self):
        """
        Delete ODF operator manager pod and verify:
        ...
        logger.info("Deleting ODF operator controller-manager pod")
        label = constants.ODF_OPERATOR_CONTROL_MANAGER_LABEL
        ...
        logger.info("Waiting for ODF operator pod to restart")
        wait_for_pods_to_be_running(
        ...
        ), "FIO produced no I/O during ODF operator restart"
        logger.info("ODF operator crash recovery verified")
