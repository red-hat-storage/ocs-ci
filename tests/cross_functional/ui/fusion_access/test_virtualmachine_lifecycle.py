...
class TestVirtualMachineLifecycle(ManageTest):
    ...
    def _login_to_vm_console(self, vm_name, namespace, vm_username, vm_password):
        ...
        index = child.expect([r"\]\$\s*", r"[Ll]ogin:\s*"], timeout=300)
        if index == 0:
    ...
    def _create_vm_and_wait_for_running(self):
        """
        Create a new namespace, navigate through the instance-type VM creation
        wizard (4.21), and wait until the new VirtualMachine reaches Running.

        Steps performed:
        1. Create a new project/namespace and select it from the All Projects
           dropdown under Workloads > Pods.
        2. Navigate to Virtualization > VirtualMachines via the left-side nav
        3. Click the Create split-button (``data-test="item-create"``) and
           select "From InstanceType" from the dropdown.
        4. Select volume to boot from: click the first available row in the
           bootable volumes table
        5. Select InstanceType: scroll to and click the General Purpose card
        6. VirtualMachine details: generate a unique VM name, clear the
           auto-filled name field
           and enter the generated name; then open the Storage class dropdown
           and pick the option whose ``id`` ends with ``-vm``.
        7. Click Create VirtualMachine in the wizard footer
        8. Wait for the VM status to reach Running.
        ...
        logger.info(
            ...
            "change Storage class to *-vm option"
        )
        logger.info("-" * 80)
    ...
    def test_create_virtualmachine_from_instancetype(self):
        """
        Test to create a VirtualMachine via the instance-type UI wizard (4.21).

        Test Steps:
        1. Create a test namespace, navigate to Workloads > Pods and select
           the namespace from the All Projects dropdown.
        2. Navigate to Virtualization > VirtualMachines via the left-side nav.
        3. Click Create split-button and select
           "From InstanceType".
        4. Select volume to boot from: click the first available row in the
           bootable volumes table.
        5. Select InstanceType: scroll to and click the General Purpose card.
        6. VirtualMachine details: enter a unique VM name and pick the Storage
           class option ending with ``-vm``.
        7. Click Create VirtualMachine in the wizard footer.
        8. Wait for VM status: Running.
        9. Fetch VM credentials (username/password) from the VM YAML,
