...
class TestNfsExport(NFSClientTestBase):
    ...
    def setup_teardown(
        ...
        # Only load NFS client credentials when not on vSphere
        ...
        if (
            ...
            # Load balancer is not supported on vSphere; skip LB creation there
            if (
        ...
        # (skipped on vSphere as there is no out-of-cluster connection)
        if (
            not cls.is_vsphere
            and hasattr(cls, "_TestNfsExport__nfs_client_connection")
    ...
    def test_nfs_export_operations_in_out_cluster(
        ...
        # TODO: Enable out-of-cluster I/O operations for vSphere once the NFS
        #       load-balancer is supported on vSphere. Currently all out-of-cluster
        #       steps (external NFS mount, cross-validation via NFS client VM) are
        #       skipped on vSphere because no external load-balancer is available.
        ...
        h) **Cluster Shutdown and Recovery:**
           - Perform ordered cluster shutdown in a controlled sequence:
             * Shut down OpenShift worker nodes first
             * Shut down OpenShift control-plane/master nodes next
           - Start the cluster in reverse order:
             * Start control-plane/master nodes and wait for cluster health
             * Start worker nodes
           - Verify ODF/Ceph components are healthy
        ...
        3. Data integrity is maintained across all operations:
           ...
           - Ordered cluster shutdown and recovery
        ...
        # On vSphere there is no load balancer so no out-of-cluster access is possible.
        # Skip the nfs_client check only when NOT on vSphere.
        ...
        # On vSphere there is no external LB/NFS client; con is None
        con = None if self.is_vsphere else self.con

        if not self.is_vsphere:
            # Mount NFS export on the external client VM
        ...
        def cleanup_all_resources():
            ...
            # 1. Unmount NFS (only when out-of-cluster connection exists)
        ...
        # I/O always runs from the in-cluster pod.
        # The pod mounts the NFS PVC at /mnt, so pod_io_file is the in-pod path.
        IO_TEST_FILE_NAME = "io_test_single.txt"
        pod_io_file = f"/mnt/{IO_TEST_FILE_NAME}"
        # nfs_io_file is used only on non-vSphere for cross-validation via the external mount
        ...
        # Ensure the I/O thread is always stopped on any exit path (pass/fail/error).
        # Without this, a test failure before the explicit io_stop_event.set() leaves
        # the thread running during teardown, hammering a pod that may already be deleted.
        def stop_io_thread():
        ...
        # Wait for Ceph cluster to recover after the node reboot.
        # OSD volume re-attachment via the CSI driver can take 13+ minutes on
        # IBM Cloud / vSphere after the node reports Ready; use tries=20, delay=60
        # (20 min budget, one poll per minute) to avoid a spurious
        # CephHealthException on slow platforms while minimising API calls.
        ...
        # Verify NFS mount is still accessible (out-of-cluster check only on non-vSphere)
        if not self.is_vsphere:
        ...
        # Step 5: Verify restored PVC data from pod; mount on NFS client only on non-vSphere
        ...
        if not self.is_vsphere:
            restored_export = self.get_nfs_export_details(restored_pvc_name)
            restored_share_details = restored_export["share_details"]
        ...
        # Pass expected values so the function asserts them on ALL platforms (including vSphere)
        verification_result = self.verify_data_integrity_from_pod(
        ...
        restored_file_checksum = verification_result["pod_data"]["checksum"]
        restored_line_count = verification_result["pod_data"]["line_count"]
        ...
        # Step 3: Mount cloned NFS export on external client (non-vSphere only)
        ...
        # Step 4: Write IO from pod (always in-cluster)
        ...
        cloned_pod_obj = cloned_pod_objs[0]
        log.info("Writing data to cloned PVC via pod...")
        ...
        # Step 5: Capture checksum before resize (from pod)
        ...
        # Step 7+8: Verify data integrity after resize — pod is source of truth on all platforms;
        # also asserts pre-resize checksum/lines are preserved, and cross-checks NFS on non-vSphere.
        log.info("Step 7: Verifying data integrity after PVC resize")
        post_resize_result = self.verify_data_integrity_from_pod(
        ...
        post_resize_checksum = post_resize_result["pod_data"]["checksum"]
        post_resize_line_count = post_resize_result["pod_data"]["line_count"]
        ...
        # Mount final restored export on NFS client only on non-vSphere
        ...
        # Step 5: Write IO from pod (always in-cluster)
        ...
        final_restored_pod_obj = final_restored_pod_objs[0]
        log.info("Writing data to final restored PVC via pod...")
        success, errors = self.write_io_to_single_file(
        ...
        # Step 6: Capture checksum before resize (from pod)
        ...
        # Step 8+9: Verify data integrity after final resize — pod is source of truth on all platforms;
        # also asserts pre-resize checksum/lines are preserved, and cross-checks NFS on non-vSphere.
        log.info("Step 8: Verifying data integrity after final PVC resize")
        post_final_result = self.verify_data_integrity_from_pod(
        ...
        post_final_resize_checksum = post_final_result["pod_data"]["checksum"]
        post_final_resize_line_count = post_final_result["pod_data"]["line_count"]
        ...
        # ========================================================================
        # Ordered Cluster Shutdown with Mount Point Validation
        # ========================================================================
        log.info("=" * 80)
        log.info("Ordered Cluster Shutdown with Mount Point Validation")
        log.info("=" * 80)
        ...
        # List of all mount points created in this test (only relevant on non-vSphere)
        nfs_mount_points = (
            [
                test_folder_for_pod,
        ...
        # List of all pod deployment names created in this test
        pod_names = [
            pod_name,
            restored_pod_name,
            cloned_pod_name,
        ...
        # Verify all mount points are accessible before shutdown (non-vSphere only)
        if not self.is_vsphere:
        ...
        # Perform ordered cluster shutdown
        ...
        # Verify Ceph health after ordered shutdown/startup
        ceph_health_check(tries=30, delay=60)
        log.info("Ceph cluster health verified after ordered shutdown")

        # Wait for NFS server pod Running and its container Ready —
        # same pattern as post-node-reboot recovery; a Running pod does not
        # guarantee the ganesha process inside is serving yet.
        ...
        # Post-recovery I/O: always from pod; also cross-check via NFS mount on non-vSphere
        log.info("Performing post-recovery I/O and data integrity verification")

        # Map pod names to their NFS mount folders (empty on vSphere)
        mount_to_pod_map = (
            {
                test_folder_for_pod: pod_name,
                restored_test_folder: restored_pod_name,
                cloned_test_folder: cloned_pod_name,
                final_restored_test_folder: final_restored_pod_name,
            }
            if not self.is_vsphere
            else {}
        )
        ...
        for pname in pod_names:
            try:
                ...
                # Cross-check via NFS mount on non-vSphere
                if not self.is_vsphere:
                    mount_point = next(
        ...
        # Final summary for ordered shutdown scenario
        log.info("=" * 80)
        log.info("Ordered Cluster Shutdown and Recovery Test - COMPLETED SUCCESSFULLY!")
        log.info("=" * 80)
        log.info("Summary:")
        log.info(
            "  - Shutdown type: ORDERED non-graceful shutdown (workers first, control-plane/master next; "
            "startup in reverse order)"
        )
        ...
        for pname in pod_names:
            log.info(f" ✓ Pod: {pname}")
        log.info("")
        log.info("All validations worked as per expectations:")
