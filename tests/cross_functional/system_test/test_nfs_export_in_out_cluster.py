...
class TestNfsExport(NFSClientTestBase):
    ...
    def test_nfs_export_operations_in_out_cluster(
        ...
        Scenario 1 — NFS Server Pod Node Reboot During Active I/O:
          Create 10Gi NFS PVC (RWX and RWO), mount in-cluster (pod). For RWX also
          mount out-of-cluster (client VM) simultaneously. Run continuous I/O during
          NFS server pod node reboot; verify no data loss.

        Scenario 2 — Snapshot and Restore:
          Create PVC snapshot, restore it, mount restored PVC in a new pod, verify
          checksums match the original data.

        Scenario 3 — Clone, Write, Resize:
          Clone the restored PVC, write I/O to the clone, resize clone to 15Gi,
          verify data integrity before and after resize.

        Scenario 4 — Snapshot Resized Clone, Restore, Resize Again:
          Snapshot the resized clone, restore it, write I/O, resize to 20Gi,
          verify data integrity.

        Scenario 5 — Non-Graceful Cluster Stop/Start (Ordered Shutdown):
          Force-stop workers then control-plane, start in reverse order; verify
          Ceph health, NFS mounts accessible, and post-recovery I/O from all pods.
        """
                ...
                if (
                    ...
                    try:
                        ...
                        for _ in TimeoutSampler(
                            timeout=60,
                            sleep=5,
                            func=lambda: con.exec_cmd(f"findmnt -M {test_folder}")[0]
                            != 0,
                        ):
                            break
                    except Exception as e:
        ...
        log.info("=" * 80)
        log.info("Scenario 1: NFS Server Pod Node Reboot During Active I/O")
        log.info("=" * 80)
        log.info("Starting continuous I/O operations on NFS mount")
        ...
        for _ in TimeoutSampler(
            ...
            sleep=2,
            func=lambda: all(
                data_sets[am]["io_thread"].is_alive()
                and len(data_sets[am]["io_errors"]) == 0
                for am in access_modes
            ),
        ):
            break
        ...
        log.info("Identifying node hosting NFS server pod")
        ...
        log.info("=" * 80)
        log.info("Scenario 2: Snapshot and Restore")
        log.info("=" * 80)

        log.info("Capturing file checksum from pod (always in-cluster)")
        ...
        log.info("Creating snapshot of NFS PVC")
        ...
        log.info("Creating new PVC from snapshot")
        ...
        log.info("Creating new pod with restored PVC")
        ...
        log.info("Verifying data integrity for restored PVC")
        ...
        log.info("Scenario 2: Snapshot and Restore completed successfully!")

        log.info("=" * 80)
        log.info("Scenario 3: Clone, Write, Resize")
        log.info("=" * 80)
        log.info("Creating clone of restored PVC")
        ...
        log.info("Creating pod deployment with cloned PVC")
        ...
        log.info("Getting NFS export details for cloned PVC")
        ...
        for access_mode in access_modes:
            ...
            log.info(f"[{access_mode}] Writing IO to cloned PVC from pod")
            ...
            log.info(f"[{access_mode}] Resizing cloned PVC from 10Gi to 15Gi")
        ...
        log.info("Scenario 3: Clone, Write, Resize completed successfully!")

        log.info("=" * 80)
        log.info("Scenario 4: Snapshot Resized Clone, Restore, Resize Again")
        log.info("=" * 80)

        FINAL_IO_FILE_NAME = "io_test_final.txt"

        log.info("Creating snapshot of resized cloned PVC")
        for access_mode in access_modes:
            cloned_pvc_name = data_sets[access_mode]["cloned_pvc_name"]
        ...
        log.info("Restoring PVC from snapshot of resized PVC")
        for access_mode in access_modes:
            cloned_pvc_name = data_sets[access_mode]["cloned_pvc_name"]
        ...
        log.info("Creating pod deployment with final restored PVC")
        for access_mode in access_modes:
        ...
        log.info("Mounting NFS, writing IO, resizing, and verifying per mode")
        for access_mode in access_modes:
            ...
            log.info(f"[{access_mode}] Writing IO to final restored PVC")
            success, errors = self.write_io_to_single_file(
            ...
            log.info(f"[{access_mode}] Capturing checksum before final resize")
        ...
        log.info(
            "Scenario 4: Snapshot Resized Clone, Restore, Resize Again completed successfully!"
        )

        log.info("=" * 80)
        log.info("Scenario 5: Non-Graceful Cluster Stop/Start (Ordered Shutdown)")
        log.info("=" * 80)

        log.info(
            "Building mount points and pod names lists from data_sets across all modes"
        )
        nfs_mount_points = []
        pod_names = []
        if not self.is_vsphere:
        ...
        log.info(
            "Verifying post-recovery I/O and data integrity across pods and NFS mounts"
        )
