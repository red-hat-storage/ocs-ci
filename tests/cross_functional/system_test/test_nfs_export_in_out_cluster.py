        ...
        try:
            ...
            while not io_stop_event.is_set():
                ...
                try:
                    ...
                    exc_str = str(exc)
                    # "container not found"  — pod restarting after node reboot
                    # "not found"            — pod fully deleted (e.g. during teardown)
                    if "container not found" in exc_str or "not found" in exc_str:
                        # Pod is temporarily or permanently unavailable.
                        # Count the whole outage window as ONE error entry and
                        # back off — do not flood io_errors with one entry per retry.
                        if not pod_unavailable:
                            ...
                            log.warning(
                                f"Pod unavailable at iteration {iteration} "
                                f"— backing off 10 s and retrying"
                            )
                            pod_unavailable = True
                        else:
                            log.debug(
                                f"Pod still unavailable at iteration {iteration}"
                                f" — waiting 10 s"
                            )
                        time.sleep(10)
                        continue
