...
class TestKeyRotationWithClusterFull(E2ETest):
    ...
    def test_cluster_wide_encryption_key_rotation(
        ...
        """
        log.info("Starting MCG setup entry criteria")
        # MCG entry criteria
        setup_mcg_bg_features(
            num_of_buckets=10,
            object_amount=10,
        ...
        log.info("Starting the CephFS setup entry criteria")
        ...
        log.info("Starting FIO run until the cluster is full")
        run_fio_till_cluster_full()
        log.info(
        ...
        verify_new_key_after_rotation(tries, delays)
        # ceph health check on rook-ceph-tool

        log.info("Initiating the NooBaa rebuild process")

        validate_noobaa_rebuild_system(bucket_factory_session, mcg_obj_session)
        log.info(
            "Verifying that the key rotation time remains unchanged after the NooBaa rebuild"
        )
        self.verify_key_rotation_time(schedule=schedule)
        log.info(
            "Verifying the newly generated key by comparing it with the older key after the NooBaa rebuild"
        )
        verify_new_key_after_rotation(tries, delays)
        ...
        log.info("Starting the validation of the NooBaa DB backup and recovery test")
        validate_noobaa_db_backup_recovery_locally_system(
            bucket_factory_session,
            noobaa_db_backup_and_recovery_locally,
            warps3,
        ...
        log.info(
            "Verifying that the key rotation time remains unchanged after the NooBaa DB backup and recovery"
        )
        self.verify_key_rotation_time(schedule=schedule)
        log.info(
            "Comparing the new key with the older key after the NooBaa DB backup and recovery"
        )
        verify_new_key_after_rotation(tries, delays)
        log.info(
            "Test execution completed for test_cluster_wide_encryption_key_rotation"
        )
