...
class TestKrknNodeScenarios:
    ...
    @pytest.mark.parametrize(
        ...
        ids=["node-stop-start", "node-reboot"],
    )
...
class TestKrknAWSNodeScenarios:
    ...
    def test_krkn_aws_node_disk_detach_attach(
        self,
        krkn_setup,
        krkn_scenario_directory,
        workload_ops,
        ...
        """
        Test AWS node disk detach/attach scenario.
        ...
        try:
            ...
            scenarios = [
                {
                    "actions": [constants.KRKN_NODE_DISK_DETACH_ATTACH],
                    "cloud_type": constants.KRKN_CLOUD_AWS,
                    "label_selector": constants.WORKER_LABEL,
                    "instance_count": 1,
                    "timeout": 120,
            ...
            validator.handle_krkn_command_failure(e, "aws", "node disk detach/attach")
            raise
        finally:
            workload_ops.validate_and_cleanup()
        ...
        validator.validate_chaos_execution(
            total_executed, successful_executed, "aws", "node disk detach/attach"
        )

        no_crashes, crash_details = health_helper.check_ceph_crashes(
            "cluster", "aws disk detach/attach"
        )
        assert no_crashes, crash_details
