        ...
        def finalizer():
            ...
            try:
                for load_status in TimeoutSampler(
                    300, 3, config.RUN.get, "load_status"
                ):
...
def awscli_pod_fixture(request):
    ...
    try:
        ...
        if "already exists" in str(e):
            log.info(
                "Leftover service CA configmap found. Trying to delete and recreate."
            )
            ocp.OCP(namespace=constants.DEFAULT_NAMESPACE, kind="configmap").delete(
                resource_name=constants.AWSCLI_SERVICE_CA_CONFIGMAP_NAME
            )
            service_ca_configmap = helpers.create_resource(
                **templating.load_yaml(constants.AWSCLI_SERVICE_CA_YAML)
            )
...
def backingstore_factory(request, cld_mgr, cloud_uls_factory):
    ...
    """
    return backingstore_factory_implementation(request, cld_mgr, cloud_uls_factory)
...
def backingstore_factory_session(request, cld_mgr, cloud_uls_factory_session):
    ...
    return backingstore_factory_implementation(
        request, cld_mgr, cloud_uls_factory_session
    )
...
def ns_resource_factory(request, mcg_obj, cld_mgr, cloud_uls_factory):
    ...
    def _create_ns_resources():
        ...
        rand_connection = create_unique_resource_name(
            constants.MCG_NS_AWS_CONNECTION, "aws"
        )
        ...
        mcg_obj.check_ns_resource_validity(
            rand_ns_resource, target_bucket_name, constants.MCG_NS_AWS_ENDPOINT
        )
