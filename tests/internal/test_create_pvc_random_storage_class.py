...
class TestCreatePVCRandomStorageClass(ManageTest):
    ...
    def test_create_pvc_with_random_sc(self, secret_factory, storageclass_factory, pvc_factory):
        sc_list = []
        ...
        # Create Rbd Sc
        for i in range(5):
            rbd_sc_obj = storageclass_factory(interface=constants.CEPHBLOCKPOOL, secret=rbd_secret_obj)
            sc_list.append(rbd_sc_obj)

        # Create CephFs Sc
        cephfs_sc_obj = storageclass_factory(interface=constants.CEPHFILESYSTEM, secret=cephfs_secret_obj)
