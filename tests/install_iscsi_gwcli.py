...
trusted_ip_list = {0}""".format(trusted_ip_list)
            ...
            for ceph in ceph_nodes:
                if ceph.role == 'osd' and check <= no_of_gateways:
                    ...
                    ceph_osd_nodes.exec_command(
                        cmd='sudo gwcli /iscsi-target/iqn.2003-01.com.redhat.iscsi-gw:ceph-igw/gateways create ' +
            ...
            for i in range(0, no_of_luns):
                ceph_osd_nodes.exec_command(sudo=True,
                                            cmd='gwcli /disks create rbd image=' + image_name + str(i) + ' size=2g')
            for ceph in ceph_nodes:
            ...
            for ceph in range(len(ceph_iscsi_initiatorname)):
                ceph_osd_nodes.exec_command(
                    cmd='sudo gwcli /iscsi-target/iqn.2003-01.com.redhat.iscsi-gw:ceph-igw/hosts create ' +
                        ceph_iscsi_initiatorname[ceph].rstrip())
                ceph_osd_nodes.exec_command(
                    cmd='sudo gwcli /iscsi-target/iqn.2003-01.com.redhat.iscsi-gw:ceph-igw/hosts/' +
                        ceph_iscsi_initiatorname[ceph].rstrip() +
                        ' auth ' + ini_name.rstrip('\n') +
                        '/redhat@123456 "|" nochap')
                log.info('Client Added ' + ceph_iscsi_initiatorname[ceph])
            multipath = \
                """devices {
                            device {
                                    vendor                 "LIO-ORG"
            ...
            log.error('No_of_gateways excited {} gateway node found'.format(no_of_gateways))
    else:
