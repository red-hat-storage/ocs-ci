...
_OCS_CI_JIRA_DIR = _ISSUE_VERIFICATION_DIR.parents[1] / "agents" / "ocs_ci_jira"
...
def generate_repro_steps_for_issue(
    ...
    *,
    ...
    """
    Generate reproduction and verification plan for a single issue via Claude.

    Claude uses full JIRA context (description + comments) for Rovo-quality steps.
    Heuristic template fallback is not used.
    """
    from claude_repro_generator import generate_repro_steps_with_claude

    enriched = merge_mcp_issue_details(issue, mcp_issue)
    topology_info = classify_topology(enriched)

    claude_result = generate_repro_steps_with_claude(
        enriched,
    ...
    analysis_notes = claude_result.get("analysis_notes") or (
        "Generated by Claude from JIRA description and comments "
        "(Rovo-equivalent analysis; no public Rovo API)."
    )
    ...
    return {
        ...
        "reproduction_steps": claude_result["reproduction_steps"],
        "expected_result": claude_result["expected_result"],
        "verification_steps": claude_result["verification_steps"],
        ...
        "fix_pull_requests": [
            {
                "url": pr.get("url"),
                "title": pr.get("title") or pr.get("jira_link_title"),
                "state": pr.get("state"),
                "source": pr.get("source"),
                "enrichment": pr.get("enrichment"),
            }
            for pr in (enriched.get("fix_pull_requests") or [])
        ],
    }
...
def refresh_issue_from_jira(
    ...
    *,
    ...
    return jira_ops.get_issue_with_fix_context(
        issue_key,
    ...
    for issue in issues:
        ...
        try:
            ...
            if mcp_payload:
                ...
                try:
                    working = refresh_issue_from_jira(
                        key,
                        jira_config=jira_config,
                        include_fix_prs=include_fix_prs,
                    )
